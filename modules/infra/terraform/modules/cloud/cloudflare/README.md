...
| `email_routing.tf` | `cloudflare_email_routing_*` + the DNS records Email Routing needs (MX ×3, SPF with Google's include merged in, DMARC p=none, zone-specific DKIM adopted from the `cloudflare_email_routing_dns` data source once Cloudflare returns it) — the addresses in `email_routing_rules` (gio@, contact@, …) each forward to their own Gmail via literal rules, and the catch-all forwards everything else to `email_routing_destination`. Needs the API token to hold `Zone/Email Routing Rules/Edit` + `Account/Email Routing Addresses/Edit`, and each destination's verification email to be clicked before its rules activate (header of the file has the full checklist) |
