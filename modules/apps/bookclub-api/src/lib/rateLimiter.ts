...
// per service (no horizontal scaling, see roomHub.ts's own comment on
// the same assumption), so "every request lands on this same
// process" always holds here. Anti-abuse throttle for an
// internet-facing endpoint, not a real traffic shaper -- mirrors
// domain-api's own ratelimiter.go (same shape: token bucket per IP,
// idle buckets evicted so this can't leak memory against an endless
// stream of distinct IPs).
