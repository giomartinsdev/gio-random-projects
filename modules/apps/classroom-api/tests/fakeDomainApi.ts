...
// A real HTTP server standing in for domain-api's Room/Message
// contract (X-API-Key auth, synchronous-for-tests writes) -- same
// "fake the other system's contract, keep everything else real"
// approach bookclub-api/post-api's own tests use. Room
// updates/deletes enforce the same host_id ownership check
// domain-worker's real aggregate does, since that's part of the
// contract too. Delete mirrors the real soft close (status ->
// "closed"), not a physical removal.
export function startFakeDomainApi(apiKey: string) {
  ...
  app.post("/rooms", async (c) => {
    ...
    if (!body.host_id || !body.title) {
      return c.json({ error: "host_id and title are required" }, 400);
    }
