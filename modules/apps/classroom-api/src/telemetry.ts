...
if (endpoint) {
  const sdk = new NodeSDK({
    ...
    instrumentations: [
      // Inbound requests (hono runs on node:http).
      new HttpInstrumentation(),
      // The pg pool better-auth reads sessions through — every query a
      // child span.
      new PgInstrumentation(),
      // Global fetch (lib/domainApiClient.ts) — stitches the
      // domain-api leg of a request (rooms, messages, and the long-lived
      // SSE subscriptions) into the same trace.
