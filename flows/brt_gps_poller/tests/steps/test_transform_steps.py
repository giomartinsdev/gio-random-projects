...
@given(parsers.parse('a well-formed BRT row for vehicle "{vehicle_id}" on line "{line}"'))
...
@given(
    parsers.parse(
        'a well-formed BRT row for vehicle "{vehicle_id}" on line "{line}" with no velocidade'
    )
)
...
@given(parsers.parse('a malformed BRT row missing "{field}"'))
...
def _when_transformed(rows: list[dict[str, Any]]) -> list[Any]:
    return BrtPositionTransformer().transform(rows)
